FROM ubuntu:20.04
ARG SOLC=VERSION

# install basic packages
RUN apt-get update && apt-get install -y\
    software-properties-common\
    locales

# set correct locale
RUN locale-gen en_US.UTF-8
ENV LANG en_US.UTF-8
ENV LANGUAGE en_US:en
ENV LC_ALL en_US.UTF-8

# install tools
RUN apt-get update &&\
        apt-get -y install\
        wget \
        gdebi \
        curl 

# install apt-utils
RUN apt-get update &&\
        apt -y install \ 
        apt-utils

# install souffle
RUN wget https://github.com/souffle-lang/souffle/releases/download/2.3/x86_64-ubuntu-2004-souffle-2.3-Linux.deb -O /tmp/souffle.deb &&\
        gdebi --n /tmp/souffle.deb

# install graphviz and pip
RUN apt update && apt -y install\
        software-properties-common
RUN add-apt-repository ppa:deadsnakes/ppa
RUN apt -y install \
        graphviz \
        python3.7 \
        python3.7-distutils \
        python3-pip

# install the required solc version
RUN curl -L https://github.com/ethereum/solidity/releases/download/v0.8.0/solc-static-linux > /usr/bin/solc-$SOLC && \
    chmod +x /usr/bin/solc-$SOLC && \
    ln -s /usr/bin/solc-$SOLC /usr/local/bin/solc

COPY requirements.txt /requirements.txt
COPY /securify /sec/securify

WORKDIR /sec

# copy and compile securify
COPY . /sec
ENV PYTHONPATH /sec

# install python module
RUN python3.7 -m pip install setuptools

# install securify requirements
RUN python3.7 setup.py install 
RUN python3.7 -m pip install --user -r requirements.txt 

RUN cd /sec/securify/staticanalysis/libfunctors/ && ./compile_functors.sh

RUN cd /sec/securify/staticanalysis/souffle_analysis && \
        souffle --dl-program=../dl-program \
        --fact-dir=/sec/securify/staticanalysis/facts_in \
        --output-dir=/sec/securify/staticanalysis/facts_out \
        -L../libfunctors -w analysis.dl


ENV LD_LIBRARY_PATH /sec/securify/staticanalysis/libfunctors

ENTRYPOINT ["python3.7", "securify/__main__.py"]